...
FROM station_names JOIN stations ON station_id = stations.sid;
...
FROM
    stations
    JOIN operators ON stations.operator_id = operators.opid
    JOIN station_names ON station_names.station_id = sid
		...
		EXCEPTION
			WHEN NO_DATA_FOUND THEN 
				...
				IF FOUND THEN 
					...
					SELECT opid INTO STRICT operator_id FROM operators WHERE operators.operator_name = NEW.operator_name;-- -*- mode: sql; sql-product: postres; sqlind-minor-mode; -*-
/* Dropping all stuff the new tables require. 
 */
DROP VIEW IF EXISTS "invalid_journeys", "completed_journeys", "departures", "arrivals";
...
BEGIN
	...
	ELSIF (TG_OP = 'UPDATE') THEN
		...
		EXCEPTION
			WHEN NO_DATA_FOUND THEN 
				...
				IF FOUND THEN 
					...
					SELECT opid INTO STRICT operator_id FROM operators WHERE operators.operator_name = NEW.operator_name; 
				ELSE 
					RAISE EXCEPTION 'Could not add new operator %', NEW.operator_name;
				END IF; 
		...
		END IF; 
		RAISE NOTICE FORMAT('Updated operator %L name to %L', NEW.operator_id, NEW.operator_name); 
		...
		IF (NEW.lang <> OLD.lang) THEN
			...
			IF NOT FOUND THEN 
				...
				IF NOT FOUND THEN 
					RAISE EXCEPTION FORMAT('Could not change the station name of station %L', OLD.sid); 
				END IF; 
			ELSE 
				-- All is fine. 
			END IF; 
